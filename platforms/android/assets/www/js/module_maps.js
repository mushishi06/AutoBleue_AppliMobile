...
var myLatlng = new google.maps.LatLng(43.699651,7.235184);
var markerCurrent = null;
MIN_ACCURACY = 10; //metres
MAX_ACCURACY = 1500
var watchID = null;
var currentCercle;
...
function getInfosModuleVersion() {
	...
	module_version = {"name":"module_maps", "version":[0,1,16] };
	...
	//document.getElementById('module_version').innerHTML = _msg;
	alert(_msg);
	...
	google.maps.event.addDomListener(controlUI, 'click', function() {
		...
		function onSuccess(position) {
			if (position.coords.accuracy > MAX_ACCURACY){
				...
				//	attachMessage(marker, results[1].formatted_address, 1);
		...
		// function(position) {	
			...
			// // geocoder.geocode({'latLng': location}, function(results, status) {			
				// // if (status == google.maps.GeocoderStatus.OK) {
					// // if (results[1]) {	
						// // currentAddr = results[1].formatted_address;
			...
			// }
			// else{
				// markerCurrent.position = location;
			// }
		// //	attachMessage(marker, results[1].formatted_address, 1);
			// map.setCenter(myLatlng);
		// },
 ...
 markers[i++] = {"lat":43.723105, "idspot":376, "lng":7.108012, "street":"4 place du Maréchal Juin", "zip":"06140", "city":"VENCE", "name":'VENCE - Maréchal Juin', "typeVehicles": " : Peugeot iOn, Utilitaire / Commercial van"};
		...
		for (i = 0; i < markers.length; i++) {
			...
			var marker = new google.maps.Marker({
				icon: new google.maps.MarkerImage("./images/gmap2.gif", null, null, null, new google.maps.Size(18, 18)),
				position: location,
				map: map
			});
			var j = i + 1;
